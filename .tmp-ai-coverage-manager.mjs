...
function mergeStateWithMatrix(matrix, existingState) {
  ...
  for (const obligation of matrix.obligations) {
    ...
    const normalizedStatus = current.status === 'baseline_existing' ? 'pending' : current.status;
  ...
  for (const id of Object.keys(state.obligations)) {
    if (!validIds.has(id)) {
      state.obligations[id].status = 'deprecated';
      state.obligations[id].lastUpdatedAt = new Date().toISOString();
    }
  }
...
function writeTestsMarkdown(matrix, state) {
  ...
  lines.push('## Summary');
  lines.push('');
  lines.push(`- Total obligations: ${summary.total}`);
  lines.push(`- Passed: ${summary.byStatus.passed || 0}`);
  lines.push(`- Pending: ${summary.byStatus.pending || 0}`);
  lines.push(`- Baseline existing: ${summary.byStatus.baseline_existing || 0}`);
  lines.push(`- Failed: ${summary.byStatus.failed || 0}`);
  lines.push(`- Duplicate: ${summary.byStatus.duplicate || 0}`);
  lines.push(`- Coverage (% passed over total): ${summary.coveragePercent}%`);
  lines.push(`- Agent coverage (% passed + baseline over agent-enabled): ${summary.agentCoveragePercent}%`);
  lines.push('');
  lines.push('## Obligations');
  lines.push('');
  lines.push('| ID | Route | Kind | Priority | Agent | Status |');
  lines.push('|---|---|---|---|---|---|');
  for (const o of obligations) {
    lines.push(`| ${o.id} | \`${o.route}\` | ${o.kind} | ${o.priority} | ${o.agentEnabled ? 'yes' : 'no'} | ${o.status} |`);
  }
...
function runSync() {
  ...
  const state = mergeStateWithMatrix(matrix, existingState);
  writeJson(COVERAGE_STATE_PATH, state);
  writeTestsMarkdown(matrix, state);
...
function runNext(format = 'text') {
  ...
  const candidates = matrix.obligations
    ...
    .filter((o) => !['passed', 'waived', 'deprecated'].includes(o.status))
    .sort((a, b) => {
      const pa = PRIORITY_ORDER.get(a.priority) ?? 9;
      const pb = PRIORITY_ORDER.get(b.priority) ?? 9;
      if (pa !== pb) return pa - pb;
