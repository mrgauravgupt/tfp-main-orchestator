...
export AIP_MODERATION_WORKER_INSTANCES="${AIP_MODERATION_WORKER_INSTANCES:-1}"
...
export COLLAGE_WORKER_POLL_INTERVAL_SECONDS="${COLLAGE_WORKER_POLL_INTERVAL_SECONDS:-10}"
export COLLAGE_WORKER_BATCH_SIZE="${COLLAGE_WORKER_BATCH_SIZE:-5}"
...
if [[ "$DEPLOY_COLLAGE" == "true" ]]; then
  ...
  bash "$ROOT_DIR/tfp-collage-service/scripts/vps/deploy-interactive.sh" "$DEPLOY_ENV"
