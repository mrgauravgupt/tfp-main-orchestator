...
while true; do
  ...
  if TARGET_ROW="$(coverage_get_next_target)"; then
    IFS=$'\t' read -r CURRENT_OBLIGATION_ID CURRENT_TARGET_ROUTE CURRENT_TARGET_GOAL <<< "$TARGET_ROW"
    log_line "Coverage target: id=${CURRENT_OBLIGATION_ID:-none} route=${CURRENT_TARGET_ROUTE:-none}"
  else
...
PROMPT_EOF
  ...
  if [ -z "$GENERATED_CODE" ]; then
    ...
    default_test "$TEST_FILE" "$FALLBACK_ROUTE"
    USED_FALLBACK=1
  ...
  if is_duplicate_content "$TEST_FILE"; then
    DUPLICATE_COUNT=$((DUPLICATE_COUNT + 1))
    coverage_mark_status "$CURRENT_OBLIGATION_ID" "duplicate" "$FALLBACK_ROUTE" "$TEST_FILE" "$FLOW_LINE" "content_hash_duplicate"
    coverage_render_dashboard || true
    log_line ""
    log_line "Sleeping for 5 minutes..."
    sleep 300
