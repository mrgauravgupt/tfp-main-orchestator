...
extract_missing_export_details() {
  local line symbol module
  line="$(grep -m1 "does not provide an export named" "$LAST_ERROR_DB" 2>/dev/null || true)"
  symbol="$(printf '%s' "$line" | sed -n "s/.*export named '\([^']*\)'.*/\1/p")"
  module="$(grep -m1 "from '" "$LAST_ERROR_DB" 2>/dev/null | sed -n "s/.*from '\([^']*\)'.*/\1/p")"
  if [ -z "$module" ]; then
